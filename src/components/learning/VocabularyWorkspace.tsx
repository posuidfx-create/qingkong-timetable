          ...
          {visibleWords.map((word, index) => {
            ...
            return (
              <article className="vocabulary-row" data-selected={selected || undefined} data-selection={selectionMode || undefined} key={word.id} onFocus={() => setFocusedWord(word)} onMouseEnter={() => setFocusedWord(word)} role="listitem">
                ...
                <button aria-label={t("vocabulary.playTerm", { term: word.term })} className="vocabulary-row-word" disabled={selectionMode} onClick={() => play(word)} type="button"><strong>{word.term}</strong>{word.reading ? <small>{word.reading}</small> : null}</button>
                ...
                <button aria-label={t(activeWordId === word.id ? "vocabulary.stopTerm" : "vocabulary.playTerm", { term: word.term })} className="vocabulary-speaker" disabled={selectionMode} onClick={() => play(word)} type="button">{activeWordId === word.id ? <><span aria-hidden="true" className="vocabulary-equalizer"><i /><i /><i /></span><Square className="size-4" /></> : <Volume2 className="size-5" />}</button>
