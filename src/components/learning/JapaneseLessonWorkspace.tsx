...
export function JapaneseLessonWorkspace({ courseOptions, lessonNumber, onBack, onOpenLesson, onOpenRoot }: Props) {
  ...
  return <section className="japanese-lesson-detail"><header className="lesson-detail-header"><button onClick={onOpenRoot} type="button">{t("vocabulary.minnaNoNihongo")}</button><div><p>LESSON {String(lessonNumber).padStart(2, "0")}</p><h1>{t("vocabulary.lessonNumber", { lesson: lessonNumber })}</h1><span>{t(volume === "beginner_1" ? "vocabulary.beginnerOne" : "vocabulary.beginnerTwo")}</span></div><Button className="lesson-picker-trigger" onClick={() => setPickerOpen(true)} variant="outline"><span>{t("vocabulary.changeLesson")}</span><ChevronDown className="size-4" /></Button><dl><div><dt>{t("vocabulary.words")}</dt><dd>{counts?.words ?? 0}</dd></div><div><dt>{t("vocabulary.grammar")}</dt><dd>{counts?.grammar ?? 0}</dd></div><div><dt>{t("vocabulary.notes")}</dt><dd>{(counts?.words ?? 0) + (counts?.grammar ?? 0)}</dd></div></dl></header><nav aria-label={t("vocabulary.lessonNavigation")} className="lesson-detail-tabs"><button data-active={lessonTab === "words"} onClick={() => setLessonTab("words")} type="button">{t("vocabulary.words")}</button><button data-active={lessonTab === "grammar"} onClick={() => setLessonTab("grammar")} type="button">{t("vocabulary.grammar")}</button><button data-active={lessonTab === "knowledge"} onClick={() => setLessonTab("knowledge")} type="button">{t("vocabulary.knowledge")}</button></nav>{lessonTab === "words" ? <VocabularyWorkspace courseOptions={courseOptions} initialLessonNumber={lessonNumber} onBack={onOpenRoot} wordFilter={lessonWordFilter} /> : lessonTab === "grammar" ? <GrammarLessonPanel lessonNumber={lessonNumber} /> : <LessonKnowledge analysis={currentAnalysis} analyzing={analyzing} lessonNumber={lessonNumber} onAnalyze={(force) => void analyzeLesson(force)} />}<Sheet onOpenChange={setPickerOpen} open={pickerOpen}><SheetContent className="responsive-bottom-sheet max-h-[88dvh] rounded-t-[28px]" side="bottom"><SheetHeader><SheetTitle>{t("vocabulary.changeLesson")}</SheetTitle><SheetDescription>{t(volume === "beginner_1" ? "vocabulary.beginnerOne" : "vocabulary.beginnerTwo")}</SheetDescription></SheetHeader><div className="lesson-picker-grid">{getLessonsForVolume(volume).map((lesson) => <button aria-current={lesson === lessonNumber ? "page" : undefined} data-active={lesson === lessonNumber} key={lesson} onClick={() => { onOpenLesson(lesson); setPickerOpen(false) }} type="button"><strong>{String(lesson).padStart(2, "0")}</strong><span>{buildLessonCounts(words, grammarItems, analyses)[lesson - 1]?.words ?? 0} · {buildLessonCounts(words, grammarItems, analyses)[lesson - 1]?.grammar ?? 0}</span></button>)}</div></SheetContent></Sheet></section>
}
