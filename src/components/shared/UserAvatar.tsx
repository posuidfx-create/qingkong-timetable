...
export function UserAvatar({ id, name, className }: { id: string; name: string; className?: string }) {
  return <span aria-hidden="true" className={cn("flex size-10 shrink-0 items-center justify-center rounded-2xl text-sm font-semibold", avatarColors[stableIndex(id)], className)}>{name.trim().slice(0, 1) || "同"}</span>
}
