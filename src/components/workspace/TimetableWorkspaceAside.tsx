...
export function TimetableWorkspaceAside({ focusedWindow, onFocus, onOpenLearning, onOpenTodos, todayTodoCount }: TimetableWorkspaceAsideProps) {
  ...
  return <aside className="timetable-workspace-aside" aria-label={t("timetable.workspaceAside")}>
    <section className="workspace-window workspace-today-window" data-focused={focusedWindow === "today" || undefined} onClick={() => onFocus("today")} onPointerMove={(event) => updateGlassPointerOrigin(event.currentTarget, event.clientX, event.clientY)}>
    ...
    <section className="workspace-window workspace-learning-window" data-focused={focusedWindow === "learning" || undefined} onClick={() => onFocus("learning")} onPointerMove={(event) => updateGlassPointerOrigin(event.currentTarget, event.clientX, event.clientY)}>
    ...
    <section className="workspace-window workspace-todo-window" data-focused={focusedWindow === "todo" || undefined} onClick={() => onFocus("todo")} onPointerMove={(event) => updateGlassPointerOrigin(event.currentTarget, event.clientX, event.clientY)}>
    ...
    <section className="workspace-window workspace-ai-window" data-focused={focusedWindow === "ai" || undefined} onClick={() => onFocus("ai")} onPointerMove={(event) => updateGlassPointerOrigin(event.currentTarget, event.clientX, event.clientY)}>
