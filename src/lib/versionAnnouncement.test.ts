...
describe("v3 release announcement", () => {
  it("uses v3.0.2 as the single release version source", () => {
    expect(APP_VERSION).toBe("3.0.2")
    expect(APP_VERSION_TAG).toBe("v3.0.2")
    expect(MAJOR_UPDATE_TITLE).toBe("v3.0.2 全新升级")
  })
  ...
  it("persists dismissal and does not show again for v3", () => {
    ...
    expect(markMajorUpdateSeen(storage)).toBe("v3.0.2")
    expect(storage.getItem(SEEN_VERSION_STORAGE_KEY)).toBe("v3.0.2")
