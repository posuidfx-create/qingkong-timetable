...
describe("v3 release announcement", () => {
  it("uses v3.3.1 as the single release version source", () => {
    expect(APP_VERSION).toBe("3.3.1")
    expect(APP_VERSION_TAG).toBe("v3.3.1")
    expect(MAJOR_UPDATE_TITLE).toBe("v3.3.1 全新升级")
  })
  ...
  it("persists dismissal and does not show again for v3", () => {
    ...
    expect(markMajorUpdateSeen(storage)).toBe("v3.3.1")
    expect(storage.getItem(SEEN_VERSION_STORAGE_KEY)).toBe("v3.3.1")
