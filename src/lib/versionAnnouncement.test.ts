...
describe("v3 release announcement", () => {
  it("uses v3.1.0 as the single release version source", () => {
    expect(APP_VERSION).toBe("3.1.0")
    expect(APP_VERSION_TAG).toBe("v3.1.0")
    expect(MAJOR_UPDATE_TITLE).toBe("v3.1.0 全新升级")
  })
  ...
  it("persists dismissal and does not show again for v3", () => {
    ...
    expect(markMajorUpdateSeen(storage)).toBe("v3.1.0")
    expect(storage.getItem(SEEN_VERSION_STORAGE_KEY)).toBe("v3.1.0")
