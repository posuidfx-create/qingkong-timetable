...
describe("Vision Learning image UI", () => {
  ...
  it("shows source transcription and deterministic review risk instead of hiding uncertain fields", () => { expect(review).toContain("sourceText"); expect(review).toContain("decideVocabularyDefaultSelection"); expect(review).toContain("recognitionReliable"); expect(review).toContain("recognitionSupplement"); expect(review).toContain("notRecognized") })
  it("offers manual crop controls and an exact-batch undo action", () => { expect(review).toContain("setCropValue"); expect(review).toContain("CropPreview"); expect(review).toContain("undoVocabularyImportBatch") })
