...
export async function getContributionAssetUrl(asset: CourseContributionAsset): Promise<string> { const { data, error } = await client().storage.from(asset.storageBucket).createSignedUrl(asset.storagePath, 60); if (error || !data.signedUrl) throw new CourseCommonsError("action_failed", error); return data.signedUrl }
