...
export function TodoPage() {
  ...
  return <section aria-labelledby="todo-page-title" className="mx-auto w-full max-w-md pb-18"><div className="flex items-center justify-between gap-3"><div><div className="flex size-11 items-center justify-center rounded-2xl bg-primary/10 text-primary"><ListTodo className="size-5" /></div><h2 id="todo-page-title" className="mt-3 text-2xl font-semibold tracking-tight">待办</h2><p className="mt-1 text-sm text-muted-foreground">个人记录与管理员发布分开保存。</p></div>{canManage && <button aria-label="发布待办" className="flex size-11 items-center justify-center rounded-2xl bg-primary text-primary-foreground shadow-sm" onClick={() => setAdminOverlay("create")} type="button"><Plus className="size-5" /></button>}</div><Tabs className="mt-5" defaultValue="mine"><TabsList className="w-full"><TabsTrigger value="mine">我的</TabsTrigger>{tabs.includes("cohort_2024") && <TabsTrigger value="cohort_2024">24级</TabsTrigger>}{tabs.includes("cohort_2025") && <TabsTrigger value="cohort_2025">25级</TabsTrigger>}</TabsList>{tabs.map((tab) => <TabsContent key={tab} value={tab}><div className="mt-4 space-y-5">{tab === "mine" && <section><h3 className="text-sm font-semibold">个人待办</h3><div className="mt-2 space-y-2.5">{localTodos.map((todo) => <TodoCard key={todo.id} courseName={todo.courseId ? courseNames.get(todo.courseId) : undefined} now={now} onDelete={(item) => deleteTodo(item.id)} onEdit={(item) => setLocalOverlay({ mode: "edit", todo: item })} onToggle={(item) => toggleTodo(item.id)} todo={todo} />)}{localTodos.length === 0 && <p className="rounded-2xl border bg-card p-4 text-center text-sm text-muted-foreground">还没有个人待办。</p>}</div></section>}<section><h3 className="text-sm font-semibold">管理员发布</h3><div className="mt-2 space-y-2.5">{visibleAdminTodos(tab).map((todo) => <AdminTodoCard canManage={canManage} completed={completedIds.has(todo.id)} key={todo.id} onDelete={() => void removeAdmin(todo)} onEdit={() => setAdminOverlay(todo)} onToggle={() => void toggleAdmin(todo)} todo={todo} />)}{visibleAdminTodos(tab).length === 0 && <p className="rounded-2xl border bg-card p-4 text-center text-sm text-muted-foreground">暂时没有这个范围的管理员待办。</p>}</div></section></div></TabsContent>)}</Tabs>{error && <p className="mt-4 rounded-xl bg-destructive/10 p-3 text-sm text-destructive" role="alert">{error}</p>}<button aria-label="新增个人待办" className="todo-fab flex size-14 items-center justify-center rounded-2xl bg-primary text-primary-foreground shadow-lg" onClick={() => setLocalOverlay({ mode: "create" })} type="button"><Plus className="size-6" /></button>{localOverlay && <TodoFormSheet courses={courses} onOpenChange={(open) => { if (!open) setLocalOverlay(undefined) }} onSave={saveLocal} open todo={localOverlay.mode === "edit" ? localOverlay.todo : undefined} />}{adminOverlay && <AdminTodoFormSheet onOpenChange={(open) => { if (!open) setAdminOverlay(undefined) }} onSave={saveAdmin} open todo={adminOverlay === "create" ? undefined : adminOverlay} />}</section>
}
