...
export function TodoPage() {
  ...
  if (!profile || !user) return <p className="text-sm text-muted-foreground">{t("todo.loading")}</p>
  ...
  const toggleAdmin = async (todo: AdminTodo) => { try { await toggleAdminTodoCompletion(todo.id, !completedIds.has(todo.id)); setCompletedIds((ids) => { const next = new Set(ids); if (next.has(todo.id)) next.delete(todo.id); else next.add(todo.id); return next }) } catch (reason) { setError(reason instanceof Error ? reason.message : t("common.error")) } }
  const removeAdmin = async (todo: AdminTodo) => { try { await deleteAdminTodo(todo.id); setAdminTodos((items) => items.filter((item) => item.id !== todo.id)) } catch (reason) { setError(reason instanceof Error ? reason.message : t("common.error")) } }

  return <section aria-labelledby="todo-page-title" className="mx-auto w-full max-w-md pb-18 md:max-w-6xl"><div className="flex items-center justify-between gap-3"><div><div className="flex size-11 items-center justify-center rounded-2xl bg-primary/10 text-primary"><ListTodo className="size-5" /></div><h2 id="todo-page-title" className="mt-3 text-2xl font-semibold tracking-tight">{t("todo.title")}</h2><p className="mt-1 text-sm text-muted-foreground">{t("todo.subtitle")}</p></div>{canManage ? <button aria-label={t("todo.publish")} className="flex min-h-11 items-center justify-center gap-2 rounded-2xl bg-primary px-3 text-sm font-semibold text-primary-foreground shadow-sm" onClick={() => setAdminOverlay("create")} type="button"><Plus className="size-5" /><span className="hidden md:inline">{t("todo.publish")}</span></button> : <button aria-label={t("todo.add")} className="hidden min-h-11 items-center justify-center gap-2 rounded-2xl bg-primary px-3 text-sm font-semibold text-primary-foreground shadow-sm md:flex" onClick={() => setLocalOverlay({ mode: "create" })} type="button"><Plus className="size-5" />{t("todo.add")}</button>}</div><Tabs className="mt-5" defaultValue="mine"><TabsList className="w-full md:max-w-md"><TabsTrigger value="mine">{t("todo.mine")}</TabsTrigger>{tabs.includes("cohort_2024") && <TabsTrigger value="cohort_2024">{t("profile.grade24")}</TabsTrigger>}{tabs.includes("cohort_2025") && <TabsTrigger value="cohort_2025">{t("profile.grade25")}</TabsTrigger>}</TabsList>{tabs.map((tab) => <TabsContent key={tab} value={tab}><div className="mt-4 space-y-5">{tab === "mine" && <section><h3 className="text-sm font-semibold">{t("todo.personal")}</h3><div className="mt-2 grid gap-2.5 md:grid-cols-2">{localTodos.map((todo) => <TodoCard key={todo.id} courseName={todo.courseId ? courseNames.get(todo.courseId) : undefined} now={now} onDelete={(item) => deleteTodo(item.id)} onEdit={(item) => setLocalOverlay({ mode: "edit", todo: item })} onToggle={(item) => toggleTodo(item.id)} todo={todo} />)}{localTodos.length === 0 && <p className="rounded-2xl border bg-card p-6 text-center text-sm text-muted-foreground md:col-span-2">{t("todo.emptyPersonal")}</p>}</div></section>}<section><h3 className="text-sm font-semibold">{t("todo.adminSection")}</h3><div className="mt-2 grid gap-2.5 md:grid-cols-2">{visibleAdminTodos(tab).map((todo) => <AdminTodoCard canManage={canManage} completed={completedIds.has(todo.id)} key={todo.id} onDelete={() => void removeAdmin(todo)} onEdit={() => setAdminOverlay(todo)} onToggle={() => void toggleAdmin(todo)} todo={todo} />)}{visibleAdminTodos(tab).length === 0 && <p className="rounded-2xl border bg-card p-6 text-center text-sm text-muted-foreground md:col-span-2">{t("todo.emptyAdmin")}</p>}</div></section></div></TabsContent>)}</Tabs>{error && <p className="mt-4 rounded-xl bg-destructive/10 p-3 text-sm text-destructive" role="alert">{error}</p>}<button aria-label={t("todo.add")} className="todo-fab flex size-14 items-center justify-center rounded-2xl bg-primary text-primary-foreground shadow-lg md:hidden" onClick={() => setLocalOverlay({ mode: "create" })} type="button"><Plus className="size-6" /></button>{localOverlay && <TodoFormSheet courses={courses} onOpenChange={(open) => { if (!open) setLocalOverlay(undefined) }} onSave={saveLocal} open todo={localOverlay.mode === "edit" ? localOverlay.todo : undefined} />}{adminOverlay && <AdminTodoFormSheet onOpenChange={(open) => { if (!open) setAdminOverlay(undefined) }} onSave={saveAdmin} open todo={adminOverlay === "create" ? undefined : adminOverlay} />}</section>
}
