...
export function ProfilePage({ onOpenStatistics, onOpenChangelog }: { onOpenStatistics: () => void; onOpenChangelog: () => void }) {
  ...
  return (
    <section className="mx-auto w-full max-w-md pb-8" aria-labelledby="profile-title">
      ...
      <section className="mt-4 rounded-[20px] border bg-card p-4 shadow-xs"><button type="button" className="flex min-h-11 w-full items-center justify-between text-left" onClick={() => setCohortOpen(true)}><span className="flex items-center gap-2 text-sm font-semibold"><UsersRound className="size-4 text-primary" />所属年级</span><span className="text-xs text-muted-foreground">{profile?.cohortYear ? `${profile.cohortYear === 2024 ? "24" : "25"}级 · 修改` : "选择年级"}</span></button></section>
